...
wm_measure = 'od'; %fa [.4, .7], ad [1, 2], rd [.4, .8], md [.6, 1.1], icvf [.4, 1], isovf [0, .3], od [.1, .4]
y_min = 0.1; y_max = 0.4;
save_figures = 'yes';
...
if ~strcmp(beh_measure, 'age')
    ...
    group(find(data_tbl.subID == 203)) = 3; % No behavioral measures were collected for subject 203.

end

if strcmp(beh_measure, 'lit')
    
    measure_childrenOnly_z = (nanmean(data_tbl.c_lit(group~=3)) - data_tbl.c_lit(group~=3))./nanstd(data_tbl.c_lit(group~=3), [], 1);
    
elseif strcmp(beh_measure, 'vm')
    
    measure_childrenOnly_z = (nanmean(data_tbl.c_vm(group~=3)) - data_tbl.c_vm(group~=3))./nanstd(data_tbl.c_vm(group~=3), [], 1);
    
elseif strcmp(beh_measure, 'fm')
        
    group(find(data_tbl.subID == 103)) = 3; % FM measures were not collected for subject 103.
    measure_childrenOnly_z = (nanmean(data_tbl.c_fm(group~=3)) - data_tbl.c_fm(group~=3))./nanstd(data_tbl.c_fm(group~=3), [], 1);
...
for t = 1:length(list_tract_ht)
    ...
    if strcmp(beh_measure, 'age')
        ...
        if t == 1; hold on; end
        [r, p, statstr, slope] = plotpartialcorr(data_tbl.cov_age(group~=3), ht(:, t), data_tbl.cov_sex(group~=3), beh_measure, ['Average ' wm_measure], [], list_tract_ht{t}, c(count, :));
        ylim([y_min y_max]);
    ...
    elseif strcmp(control_age, 'yes')
        ...
        if t == 1; hold on; end
        [r, p, statstr, slope] = plotpartialcorr(measure_childrenOnly_z, z_ht(:, t), [z_cov_age_childrenOnly data_tbl.cov_sex(group~=3)], beh_measure, ['Average ' wm_measure, ', z-scored'], [], list_tract_ht{t}, c(count, :));
        xlim([-2 2]);
        ylim([-2 2]);
        ...
        scatter(measure_childrenOnly_z, z_ht(:, t), 'filled', 'MarkerFaceColor', c(count, :))
        if t == 1; hold on; end
        [r, p, statstr, slope] = plotcorr2(measure_childrenOnly_z, z_ht(:, t), beh_measure, ['Average ' wm_measure, ', z-scored'], [], list_tract_ht{t}, c(count, :), .2);
        xlim([-2 2]);
        ylim([-2 2]);
...
for t = 1:length(list_tract_vt)
    ...
    if strcmp(beh_measure, 'age')
        ...
        if t == 1; hold on; end
        [r, p, statstr, slope] = plotpartialcorr(data_tbl.cov_age(group~=3), vt(:, t), data_tbl.cov_sex(group~=3), beh_measure, ['Average ' wm_measure], [], list_tract_vt{t}, c(count, :));
        ylim([y_min y_max]);
    ...
    elseif strcmp(control_age, 'yes')
        ...
        if t == 1; hold on; end
        [r, p] = plotpartialcorr(measure_childrenOnly_z, z_vt(:, t), [z_cov_age_childrenOnly data_tbl.cov_sex(group~=3)], beh_measure, ['Average ' wm_measure, ', z-scored'], [], list_tract_vt{t}, c(count, :));
        xlim([-2 2]);
        ylim([-2 2]);
        ...
        scatter(measure_childrenOnly_z, z_vt(:, t), 'filled', 'MarkerFaceColor', c(count, :))
        if t == 1; hold on; end
        [r, p, statstr, slope] = plotcorr2(measure_childrenOnly_z, z_vt(:, t), beh_measure, ['Average ' wm_measure, ', z-scored'], [], list_tract_vt{t}, c(count, :), .2);
        xlim([-2 2]);
        ylim([-2 2]);
...
t_out.region = cat(1, list_tract_ht', list_tract_vt');
t_out.r = cat(1, r_h, r_v);
t_out.p = cat(1, p_h, p_v); 
t_out_tbl = struct2table(t_out);
writetable(t_out_tbl, fullfile(rootDir, 'supportFiles', ['LWX_data_' wm_measure '_' beh_measure '_correlation.csv']));
...
for t = 1:length(list_tract_ht)
    ...
    elseif strcmp(control_age, 'yes')
        ...
        % Plot only data points that would have been included in the corelation.
        scatter(measure_childrenOnly_z, z_ht(:, t), 'filled', 'MarkerFaceColor', c(count, :))
        [r, p, statstr] = plotpartialcorr(measure_childrenOnly_z, z_ht(:, t), [z_cov_age_childrenOnly data_tbl.cov_sex(group~=3)], beh_measure, ['Average ' wm_measure, ', z-scored'], [], list_tract_ht{t}, c(count, :));
        xlim([-2 2]);
        ylim([-2 2]);
...
for t = 1:length(list_tract_vt)
    ...
    elseif strcmp(control_age, 'yes')
        ...
        % Plot only data points that would have been included in the corelation.
        scatter(measure_childrenOnly_z, z_vt(:, t), 'filled', 'MarkerFaceColor', c(count, :))
        [r, p, statstr] = plotpartialcorr(measure_childrenOnly_z, z_vt(:, t), [z_cov_age_childrenOnly data_tbl.cov_sex(group~=3)], beh_measure, ['Average ' wm_measure, ', z-scored'], [], list_tract_vt{t}, c(count, :));
        xlim([-2 2]);
        ylim([-2 2]);
...
if strcmp(beh_measure, 'age')
    ...
    % Horizontal
    figure(x)
    scatter(repmat(data_tbl.cov_age(group~=3), [size(ht, 2) 1]), ht(:), 'filled', 'MarkerFaceColor', c(count, :), 'MarkerFaceAlpha', 0.2)
    hold on;
%     scatter(repmat(max(data_tbl.cov_age(group~=3) + 4), size(ht_adult(:))), ht_adult(:), 'MarkerEdgeColor', c(count, :))
    [r, p, statstr, slope] = plotpartialcorr(repmat(data_tbl.cov_age(group~=3), [size(ht, 2) 1]), ht(:), repmat(data_tbl.cov_sex(group~=3), [size(ht, 2) 1]), beh_measure, ['Average ' wm_measure], [], 'Horizontal', c(count, :));
%     ylim([y_min y_max]);
...
%     hold off;
    ...
    % Vertical
%     figure(x+1)
    scatter(repmat(data_tbl.cov_age(group~=3), [size(vt, 2) 1]), vt(:), 'filled', 'MarkerFaceColor', c(count-10, :), 'MarkerFaceAlpha', 0.2)
    hold on;
%     scatter(repmat(max(data_tbl.cov_age(group~=3) + 4), size(vt_adult(:))), vt_adult(:), 'MarkerEdgeColor', c(count+30, :))
    [r, p, statstr, slope] = plotpartialcorr(repmat(data_tbl.cov_age(group~=3), [size(vt, 2) 1]), vt(:), repmat(data_tbl.cov_sex(group~=3), [size(vt, 2) 1]), beh_measure, ['Average ' wm_measure], [], 'Vertical', c(count-10, :));
    ylim([y_min y_max]);
...
elseif strcmp(control_age, 'yes')
    ...
    % Horizontal
    figure(x)
    scatter(repmat(measure_childrenOnly_z, [size(z_ht, 2) 1]), z_ht(:), 'filled', 'MarkerFaceColor', c(count, :), 'MarkerFaceAlpha', 0.2)
    hold on;
    [r, p, statstr, slope] = plotpartialcorr(repmat(measure_childrenOnly_z, [size(z_ht, 2) 1]), z_ht(:), repmat([z_cov_age_childrenOnly data_tbl.cov_sex(group~=3)], [size(ht, 2) 1]), beh_measure, ['Average ' wm_measure, ', z-scored'], [], 'Horizontal', c(count, :));
%     xlim([-2 2]);
%     ylim([-2 2]);
%     title(['Horizontal Tracts, slope = ' num2str(slope) ', r = ' num2str(r) ', p = ' num2str(p)]);
%     hold off;
    ...
    % Vertical
%     figure(x+1)
    scatter(repmat(measure_childrenOnly_z, [size(z_vt, 2) 1]), z_vt(:), 'filled', 'MarkerFaceColor', c(count-10, :), 'MarkerFaceAlpha', 0.2)
%     hold on;
    [r, p, statstr, slope] = plotpartialcorr(repmat(measure_childrenOnly_z, [size(z_vt, 2) 1]), z_vt(:), repmat([z_cov_age_childrenOnly data_tbl.cov_sex(group~=3)], [size(vt, 2) 1]), beh_measure, ['Average ' wm_measure, ', z-scored'], [], 'Vertical', c(count-10, :));
    xlim([-2 2]);
    ylim([-2 2]);
%     title(['Vertical Tracts, slope = ' num2str(slope) ', r = ' num2str(r) ', p = ' num2str(p)]);
%     hold off;
    ...
    scatter(measure_childrenOnly_z, z_ht(:, t), 'filled', 'MarkerFaceColor', c(count, :))
    hold on;
    [r, p, statstr, slope] = plotcorr2(repmat(measure_childrenOnly_z, [size(z_ht, 2) 1]), z_ht(:), beh_measure, ['Average ' wm_measure, ', z-scored'], [], 'Horizontal', c(count, :));
    xlim([-2 2]);
    ylim([-2 2]);
    title(['Horizontal Tracts, slope = ' num2str(slope) ', r = ' num2str(r) ', p = ' num2str(p)]);
    hold off;
    ...
    % Vertical
    figure(x+1)
    scatter(measure_childrenOnly_z, z_vt(:, t), 'filled', 'MarkerFaceColor', c(count-10, :))
    hold on;
    [r, p, statstr, slope] = plotcorr2(repmat(measure_childrenOnly_z, [size(z_vt, 2) 1]), z_vt(:), beh_measure, ['Average ' wm_measure, ', z-scored'], [], 'Vertical', c(count-10, :));
    xlim([-2 2]);
    ylim([-2 2]);
    title(['Vertical Tracts, slope = ' num2str(slope) ', r = ' num2str(r) ', p = ' num2str(p)]);
    hold off;
...
title({'Horizontal Tracts (blue) vs. Vertical Tracts (red)', ['t(' num2str(stats.df) ') = ' num2str(stats.tstat) ', p = ' num2str(p)]})
if strcmp(save_figures, 'yes')
    
    print([rootDir 'plots/plot_correlation_' beh_measure '_' wm_measure '_HvV'], '-dpng')
    print([rootDir 'plots/eps/plot_correlation_' beh_measure '_' wm_measure '_HvV'], '-depsc')
